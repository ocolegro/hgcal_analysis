...
void root_to_txt()
{
  ...
  for (unsigned int i = 0; i < sizeof(energies)/sizeof(energies[0]); i++)
  {
    for (unsigned int j = 0; j < runs.size(); j++)
    {
	    ...
	    try{
        freopen( "../data/merged_txts_2/" + out_file, "w", stdout );
        ...
        std::cout << "A file failed" << std::endl;
        }
    }
  }
}
