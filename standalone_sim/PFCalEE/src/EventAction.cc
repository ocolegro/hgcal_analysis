			 ...
			 const HGCSSGenParticle & genPart)
{
  ...
  //genPart.Print(std::cout);
  if (genPart.isIncoming()) genvec_.push_back(genPart);}
  //std::cout << "The gen part is " << genPart << std::endl;
//  std::cout << "The particle  is " << genPart.Print(std::cout) << std::endl;
//  genPart.Print(std::cout);
...
bool EventAction::isFirstVolume(const std::string volname) const{
  ...
  //    std::cout << volname.c_str() << " next " << ((*detector_)[0].ele_vol[(*detector_)[0].n_elements*iS])->GetName() << std::endl;
