...
{    
  ...
  //file >> nx >> ny >> nz; // Note dodgy order
    ...
    while(getline(file, line))
        ++n_lines;
    std::cout << "Number of lines in data file: " << n_z << endl;
/*
 ...
 */
   ...
   */
  double bval, xval;
  for (iz = nz; iz < 2*nz; iz++){
    ix = 0, iy = 0;
    do file >> zval; // Read in the z-coordinate
    while(ix < nx){
    // Read in all 25 b-field values along the x-axis
        ...
        xField[ix][ny][iz]   = 0.0  * fieldUnit;
        xField[-ix][ny][iz]  = xField[ix][ny][iz];
        xField[ix][ny][-iz]  = xField[ix][ny][iz];
        xField[-ix][ny][-iz] = xField[ix][ny][iz];

        yField[ix][ny][iz]   = bval * fieldUnit;
        yField[-ix][ny][iz]  = yField[ix][ny][iz];
        yField[ix][ny][-iz]  = yField[ix][ny][iz];
        yField[-ix][ny][-iz] = yField[ix][ny][iz];

        zField[ix][ny][iz]   = 0.0 * fieldUnit;
        zField[-ix][ny][iz]  = zField[ix][ny][iz];
        zField[ix][ny][-iz]  = zField[ix][ny][iz];
        zField[-ix][ny][-iz] = zField[ix][ny][iz];
        ++ix;
